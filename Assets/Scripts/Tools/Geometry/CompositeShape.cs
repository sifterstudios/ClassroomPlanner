...
namespace Sifter.Tools.Geometry
{
    public partial class CompositeShape
    {
        ...
        public void Process()
        {
            ...
            CompositeShape.CompositeShapeData[] eligibleShapes = shapes.Select(x => new CompositeShape.CompositeShapeData(x._points
                .ToArray())).Where(x => x.IsValidShape).ToArray();
            ...
            CompositeShape.CompositeShapeData[] holeShapes = eligibleShapes.Where(x => x.parents.Count % 2 != 0).ToArray();
            foreach (CompositeShape.CompositeShapeData holeShape in holeShapes)
            {
                ...
                CompositeShape.CompositeShapeData immediateParent = holeShape.parents.OrderByDescending(x => x.parents.Count).First();
                immediateParent.holes.Add(holeShape);
            }

            // Solid shapes have an even number of parents
            CompositeShape.CompositeShapeData[] solidShapes = eligibleShapes.Where(x => x.parents.Count % 2 == 0).ToArray();
